...
First, we want each half to have a baseline of role-players. To this end, we will include in each half:
- One boardwipe
- Three pieces of interaction (e.g. target removal or counterspell)
- Two pieces of ramp
- Two cards that generate card advantage.

When two halves are shuffled together, the result should feel like a cohesive whole. With this goal in mind, we want to place cards in each half that play well with other halves. Specifically, each half will have five cards chosen to synergize with each other half. For example, the white half will have five cards that work well with the blue half, five for the black half, and so on for the red and green halves. This leaves 22 cards: five cards bolstering the "core" strategy of the half and 17 lands. <button id="skeletontoggle">Toggle Skeleton Spreadsheet</button>
